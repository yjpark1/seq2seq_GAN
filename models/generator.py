...
class Seq2SeqGenerator:
    ...
    def build_encoder(self, inputs):
        ...
        with tf.variable_scope('encode'):
            lstm_fw_cell = tf.nn.rnn_cell.LSTMCell(num_units=self.enc_units//2, state_is_tuple=True)
            lstm_fw_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_fw_cell, output_keep_prob=0.5)
            lstm_bw_cell = tf.nn.rnn_cell.LSTMCell(num_units=self.enc_units//2, state_is_tuple=True)
            lstm_bw_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_bw_cell, output_keep_prob=0.5)

            outputs, states = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, embed, dtype=tf.float32)
...
if __name__ == '__main__':
    model = Seq2SeqGenerator(vocab_size=100,
                             ...
                             dec_units=128,
                             batch_size=32)
    inputs = tf.placeholder(tf.int32, shape=[32, 16,])
